...
let state = {
  ...
  fiveThreeOneRoundStep: parseFloat(localStorage.getItem('fiveThreeOneRoundStep')) || 2.5,
  ...
  // 개인 설정
  weightStep: parseFloat(localStorage.getItem('weightStep')) || 2.5,
  weightUnit: localStorage.getItem('weightUnit') || 'kg',
  settingsTab: 'general' // general, personal
};
...
// iOS Chrome: passive: false on global touchmove blocks ALL scrolling.
// Use passive listener globally; only add non-passive during active drag.
function handleGlobalMoveDrag(e) {
...
// Start drag: attach non-passive listeners
function startDragListeners() {
  document.addEventListener('touchmove', handleGlobalMoveDrag, { passive: false });
  document.addEventListener('mousemove', handleGlobalMoveDrag, { passive: false });
}

// Stop drag: remove non-passive listeners
function stopDragListeners() {
  document.removeEventListener('touchmove', handleGlobalMoveDrag);
  document.removeEventListener('mousemove', handleGlobalMoveDrag);
}
...
async function handleGlobalEnd(e) {
  ...
  stopDragListeners(); // Remove non-passive touchmove listener to restore normal scrolling
  const list = document.querySelector('.workout-list');
...
function render() {
  ...
  else if (state.view === 'kg-lb-converter') renderKgLbConverter();
...
async function exportAllData() {
  ...
  const inbody = await getAllRecords('inbody');

  // localStorage 설정 전체 포함
  const settingsKeys = [
    'themeMode', 'themeColor',
    'weightUnit', 'weightStep',
    'fiveThreeOneRms', 'fiveThreeOneOption', 'fiveThreeOneRoundStep',
    'oneRmInput'
  ];
  const settings = {};
...
async function importAllData() {
  ...
  input.addEventListener('change', async (e) => {
    ...
    try {
      ...
      // IndexedDB 복원
      const storeNames = ['exercises', 'groups', 'workouts', 'inbody'];
      for (const storeName of storeNames) {
        ...
        for (const record of records) { await putRecord(storeName, record); }
      }
      ...
      if (data.settings && typeof data.settings === 'object') {
        ...
        // state에 즉시 반영
        state.weightUnit = localStorage.getItem('weightUnit') || 'kg';
        state.weightStep = parseFloat(localStorage.getItem('weightStep')) || 2.5;
        state.fiveThreeOneRms = JSON.parse(localStorage.getItem('fiveThreeOneRms')) || { squat: 0, deadlift: 0, bench: 0, ohp: 0 };
        state.fiveThreeOneOption = localStorage.getItem('fiveThreeOneOption') || 'A';
        state.fiveThreeOneRoundStep = parseFloat(localStorage.getItem('fiveThreeOneRoundStep')) || 2.5;
        applyTheme();
      }

      // state 재로드
      await loadData();
      alert('데이터와 설정이 복원되었습니다.');
      navigateTo('calendar');
    } catch (err) {
...
function renderSettings() {
  ...
  const currentWeightStep = state.weightStep;

  const colors = [
    { id: 'red', hex: '#EF4444' }, { id: 'orange', hex: '#F97316' },
    { id: 'yellow', hex: '#EAB308' }, { id: 'green', hex: '#10B981' },
  ...
  const generalTabContent = `
    <div class="settings-section">
      ...
      <div style="display: flex; flex-direction: column; gap: 8px;">
        <button class="btn btn-secondary" id="manageExGrpBtn" style="justify-content: flex-start; padding: 10px; font-size: 0.95rem;"><i class="ph ph-list-dashes" style="font-size: 1.2rem; min-width: 24px;"></i> 종목 / 그룹 관리</button>
        <button class="btn btn-secondary" id="openFiveThreeOneBtn" style="justify-content: flex-start; padding: 10px; font-size: 0.95rem;"><i class="ph ph-calculator" style="font-size: 1.2rem; min-width: 24px;"></i> 5/3/1 계산기</button>
        <button class="btn btn-secondary" id="openOneRmCalcBtn" style="justify-content: flex-start; padding: 10px; font-size: 0.95rem;"><i class="ph ph-target" style="font-size: 1.2rem; min-width: 24px;"></i> 1RM 추정</button>
        <button class="btn btn-secondary" id="openKgLbConverterBtn" style="justify-content: flex-start; padding: 10px; font-size: 0.95rem;"><i class="ph ph-scales" style="font-size: 1.2rem; min-width: 24px;"></i> kg-lbs 변환기</button>
        <button class="btn btn-secondary" id="openInBodyBtn" style="justify-content: flex-start; padding: 10px; font-size: 0.95rem;"><i class="ph ph-heartbeat" style="font-size: 1.2rem; min-width: 24px;"></i> 인바디 기록 관리</button>
        <button class="btn btn-secondary" id="openWorkoutReportBtn" style="justify-content: flex-start; padding: 10px; font-size: 0.95rem;"><i class="ph ph-chart-bar" style="font-size: 1.2rem; min-width: 24px;"></i> 종합 운동 리포트</button>
        <button class="btn btn-secondary" id="openExerciseReportBtn" style="justify-content: flex-start; padding: 10px; font-size: 0.95rem;"><i class="ph ph-chart-line-up" style="font-size: 1.2rem; min-width: 24px;"></i> 개별 종목 리포트</button>
      </div>
    </div>
    <div class="settings-section">
      <div class="settings-title">데이터 관리</div>
      <div class="data-mgmt-buttons">
        <button class="btn data-mgmt-btn" id="exportDataBtn"><i class="ph ph-download-simple"></i> 데이터 백업</button>
        <button class="btn data-mgmt-btn" id="importDataBtn"><i class="ph ph-upload-simple"></i> 데이터 복원</button>
      </div>
    </div>
    <div class="settings-section">
      ...
      <div style="display: flex; flex-direction: column; gap: 8px;">
        ...
        ${("Notification" in window) && Notification.permission !== 'granted' ? `<button class="btn btn-secondary" id="requestNotiPermBtn" style="padding: 10px; font-size: 0.9rem;">알림 권한 요청하기</button>` : ''}
        ${!("Notification" in window) ? `<p style="font-size: 0.75rem; color: var(--danger); margin: 0;">현재 브라우저는 알림을 직접 지원하지 않습니다.</p>` : ''}
        <button class="btn btn-secondary" id="testNotiBtn" style="padding: 10px; font-size: 0.9rem;">알림 및 비프음 테스트</button>
  ...
  const personalTabContent = `
    ...
    <div class="settings-section">
      <div class="settings-title">무게 단위</div>
      <p style="font-size: 0.8rem; color: var(--text-muted); margin-bottom: 12px;">운동 진행 화면에서 표시할 무게 단위를 선택합니다.</p>
      <div class="mode-toggle">
        <div class="mode-btn unit-btn ${currentWeightUnit === 'kg' ? 'active' : ''}" data-unit="kg">kg (킬로그램)</div>
        <div class="mode-btn unit-btn ${currentWeightUnit === 'lbs' ? 'active' : ''}" data-unit="lbs">lbs (파운드)</div>
      </div>
    </div>
    <div class="settings-section" style="border-bottom: none;">
      <div class="settings-title">무게 증감 단위</div>
      <p style="font-size: 0.8rem; color: var(--text-muted); margin-bottom: 12px;">운동 진행 화면에서 +/- 버튼으로 한 번에 증감할 무게 값입니다.</p>
      <div style="display: flex; align-items: center; gap: 12px;">
        <input type="number" id="weightStepInput" class="form-control" value="${currentWeightStep}" min="0.25" step="0.25" style="flex: 1; text-align: center; font-size: 1.3rem; font-weight: 700;">
        <span style="font-size: 1rem; color: var(--text-muted); white-space: nowrap;">${currentWeightUnit}</span>
      </div>
      <div style="display: flex; gap: 8px; margin-top: 10px; flex-wrap: wrap;">
        ${[0.5, 1, 2.5, 5, 10].map(v => `<button class="btn preset-step-btn" data-step="${v}" style="flex: 1; min-width: 50px; padding: 8px; font-size: 0.9rem; ${currentWeightStep === v ? 'background: var(--accent-primary); color: white; border-color: var(--accent-primary);' : ''}">${v}</button>`).join('')}
      </div>
      <button class="btn btn-primary" id="saveWeightStepBtn" style="margin-top: 14px;">저장</button>
    </div>`;
  ...
  appEl.innerHTML = `
    ...
    <main>
      <div class="tabs" style="margin-bottom: 0; padding: 0 4px;">
        <div class="tab ${tab === 'general' ? 'active' : ''}" id="tabGeneral">기능</div>
        <div class="tab ${tab === 'personal' ? 'active' : ''}" id="tabPersonal">설정</div>
      </div>
      <div class="glass-panel" style="padding: 20px; border-top-left-radius: 0; border-top-right-radius: 0;">
        ${tab === 'general' ? generalTabContent : personalTabContent}
      </div>
    </main>
  `;
  ...
  safeAddListener('tabGeneral', 'click', () => { state.settingsTab = 'general'; renderSettings(); });
  safeAddListener('tabPersonal', 'click', () => { state.settingsTab = 'personal'; renderSettings(); });
  ...
  if (tab === 'general') {
    ...
    safeAddListener('manageExGrpBtn', 'click', () => navigateTo('add-exercise', { addExerciseSourceView: 'settings' }));
    ...
    safeAddListener('openKgLbConverterBtn', 'click', () => navigateTo('kg-lb-converter'));
    safeAddListener('openInBodyBtn', 'click', () => navigateTo('inbody-list'));
    safeAddListener('openWorkoutReportBtn', 'click', () => navigateTo('workout-report', { reportDate: toDateStr(new Date()) }));
    ...
    safeAddListener('testNotiBtn', 'click', () => { initAudio(); playTimerEndNotification(); });
  } else {
    // 테마 모드
    ...
    document.querySelectorAll('.preset-step-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        const step = parseFloat(e.currentTarget.getAttribute('data-step'));
        state.weightStep = step;
        localStorage.setItem('weightStep', step);
        renderSettings();
      });
    });
    safeAddListener('saveWeightStepBtn', 'click', () => {
      const input = document.getElementById('weightStepInput');
      const val = parseFloat(input ? input.value : 2.5);
      if (isNaN(val) || val <= 0) { alert('올바른 값을 입력해 주세요.'); return; }
      state.weightStep = val;
      localStorage.setItem('weightStep', val);
      renderSettings();
    });
  }
}
...
function renderCalendar() {
  ...
  // Calculate daily volumes for heatmap
  const dailyVolumes = new Array(daysInMonth + 1).fill(0);
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (w && w.exercises && w.exercises.length > 0) {
      ...
      w.exercises.forEach(ex => {
        ex.sets.forEach(s => {
          if (s.completed && s.weight > 0 && s.reps > 0) dVol += s.weight * s.reps;
        });
      });
      dailyVolumes[day] = dVol;
      if (dVol > maxDailyVolume) maxDailyVolume = dVol;
    }
  }
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    let style = '';
    if (dailyVolumes[day] > 0 && maxDailyVolume > 0) {
      const ratio = dailyVolumes[day] / maxDailyVolume;
      const opacity = 0.2 + (ratio * 0.8); // 0.2 to 1.0
      const textColor = opacity > 0.5 ? '#ffffff' : 'var(--text-primary)';
      style = `background-color: color-mix(in srgb, var(--accent-primary) ${Math.round(opacity * 100)}%, transparent) !important; color: ${textColor} !important; border-color: transparent !important;`;
    }

    gridHtml += `<div class="calendar-day ${classes}" data-date="${curDateStr}" style="${style}">${day}</div>`;
  }
  ...
  if (selectedWorkout && selectedWorkout.exercises.length > 0) {
    ...
    selectedWorkout.exercises.forEach(exItem => {
      ...
      dayTotalVolume += totalVolume;
      dayTotalSets += exItem.sets.length;

      const volumeStr = totalVolume > 0 ? `${totalVolume.toLocaleString()}${state.weightUnit}` : '-';

      previewHtml += `<li style="margin-bottom: 8px; display: flex; align-items: center; gap: 8px;">
        <div style="background: ${dotColor}; width: 8px; height: 8px; border-radius: 50%; box-shadow: 0 0 4px ${dotColor}; flex-shrink: 0;"></div>
        <span style="flex: 1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${exName}</span>
        <span style="font-size: 0.85rem; color: var(--text-secondary); font-weight: 500; text-align: right;">${volumeStr}</span>
        <span style="font-size: 0.85rem; color: var(--text-muted); font-weight: 500; min-width: 45px; text-align: right;">${exItem.sets.length}세트</span>
      </li>`;
    });

    previewHtml += `
      <li style="margin-top: 12px; padding-top: 12px; border-top: 1px solid var(--border-color); display: flex; align-items: center; gap: 8px;">
        <span style="flex: 1; font-weight: 700; color: var(--text-primary);">오늘의 총합</span>
        <span style="font-size: 0.95rem; font-weight: 700; color: var(--accent-primary); text-align: right;">${dayTotalVolume > 0 ? dayTotalVolume.toLocaleString() + state.weightUnit : '-'}</span>
        <span style="font-size: 0.9rem; font-weight: 700; color: var(--text-primary); min-width: 45px; text-align: right;">${dayTotalSets}세트</span>
      </li>
    </ul></div>`;
  } else {
  ...
  // --- 캘린더 스와이프 제스처 (좌우 슬라이드로 월 이동) ---
  const calPanel = appEl.querySelector('.glass-panel');
  if (calPanel) {
    ...
    calPanel.addEventListener('touchmove', (e) => {
      if (!calSwiping) return;
      const dx = e.touches[0].clientX - calSwipeStartX;
      const dy = e.touches[0].clientY - calSwipeStartY;
      // 가로 방향 스와이프인 경우 세로 스크롤 막기
    ...
    calPanel.addEventListener('touchend', (e) => {
      ...
      // 가로 스와이프가 세로보다 크고, 최소 50px 이상일 때만 월 이동
        ...
        setsHtml += `
          <div class="set-row">
            ...
            <div class="inline-input-group">
              <input type="number" class="inline-input weight-input" data-ex="${exIdx}" data-set="${setIdx}" value="${set.weight}" step="${state.weightStep}" min="0">
              <span class="unit-label">${state.weightUnit}</span>
            </div>
            <div class="inline-input-group">
        ...
        <div class="glass-panel exercise-card" data-ex="${exIdx}">
          <div class="exercise-header">
            ...
            <div class="exercise-actions">
              ...
              <button class="chevron-btn goto-exec-btn" data-ex="${exIdx}" aria-label="운동 진행">
  ...
  document.querySelectorAll('.exercise-card').forEach(card => {
    ...
    const handleStart = (e) => {
      ...
      dragData.dragTimer = setTimeout(() => {
        ...
        startDragListeners(); // Attach non-passive touchmove only during drag
...
function handleTimerTick() {
  if (state.timerState === 'running' && state.timerEndTime) {
    ...
    if (remaining !== state.timerTime) {
      state.timerTime = remaining;
...
function renderExecution() {
  ...
  appEl.innerHTML = `
    <header>
      ...
      <h1>진행 중</h1>
      <button class="icon-btn" id="execReportBtn" aria-label="리포트"><i class="ph ph-chart-line-up"></i></button>
    </header>
    <main class="no-scroll" style="display: flex; flex-direction: column; justify-content: space-between; min-height: 0;">
      <div class="execution-container">
        <h2 class="execution-title">${exName}</h2>
        <div class="execution-set">${currentSetNum}세트</div>
        <div class="execution-metrics">
          <div class="metric-box">
            <button class="stepper-btn" id="weightUpBtn">+${state.weightStep}</button>
            <div class="metric-value">
              <input type="number" id="execWeightInput" class="exec-input fit-text" value="${set.weight}" step="${state.weightStep}" min="0">
              <span class="metric-unit">${state.weightUnit}</span>
            </div>
            <button class="stepper-btn stepper-down" id="weightDownBtn">-${state.weightStep}</button>
          </div>
          <div style="font-size: 1.5rem; color: var(--text-muted); align-self: center;">/</div>
          <div class="metric-box">
            <button class="stepper-btn" id="repsUpBtn">+1</button>
            <div class="metric-value">
              <input type="number" id="execRepsInput" class="exec-input fit-text" value="${set.reps}" min="0">
              <span class="metric-unit">회</span>
            </div>
            <button class="stepper-btn stepper-down" id="repsDownBtn">-1</button>
          </div>
        </div>
        
        <div style="margin-top: 30px;">
          <div class="rest-timer-wrapper" style="margin: 0 auto;">
            <svg class="rest-timer-svg" viewBox="0 0 160 160" style="width: 110px; height: 110px;">
          ...
          <div style="display: flex; justify-content: center; gap: 12px; margin-top: 10px;">
            <button class="action-btn" id="sub30Btn" style="background: var(--surface); border: 1px solid var(--border-color); border-radius: 20px; padding: 6px 12px; font-size: 0.9rem; font-weight: 600; color: var(--text-primary);">-30초</button>
            <button class="action-btn" id="add30Btn" style="background: var(--surface); border: 1px solid var(--border-color); border-radius: 20px; padding: 6px 12px; font-size: 0.9rem; font-weight: 600; color: var(--text-primary);">+30초</button>
          </div>

          <div id="timerControlsRow" style="min-height: 44px; display:flex; justify-content:center; margin-top: 10px;">
          </div>
        </div>
      </div>

      <div style="display: flex; flex-direction: column; align-items: center; gap: 10px;">
        <div class="dots-indicator" style="display: flex; gap: 8px; max-width: 100%; overflow-x: auto; padding: 5px 4px; scroll-snap-type: x mandatory; scrollbar-width: none;">
          ${dotsHtml}
        </div>
  ...
  document.getElementById('execReportBtn').addEventListener('click', () => {
    state.reportSourceView = 'execution';
    navigateTo('report', { reportExerciseId: exItem.exerciseId });
  });
  ...
  // 횟수 스텝퍼 버튼
  const repsInput = document.getElementById('execRepsInput');
  document.getElementById('repsUpBtn').addEventListener('click', async () => {
    const cur = parseInt(repsInput.value) || 0;
    repsInput.value = cur + 1;
    exItem.sets[state.activeSetIndex].reps = cur + 1;
    await saveCurrentWorkout();
  });
  document.getElementById('repsDownBtn').addEventListener('click', async () => {
    const cur = parseInt(repsInput.value) || 0;
    const next = Math.max(0, cur - 1);
    repsInput.value = next;
    exItem.sets[state.activeSetIndex].reps = next;
    await saveCurrentWorkout();
  });
  ...
  // --- Execution 스와이프 제스처 (좌우 슬라이드로 세트 이동) ---
  const execMain = appEl.querySelector('main');
  if (execMain) {
    let execSwipeStartX = 0;
    let execSwipeStartY = 0;
    let execSwiping = false;

    execMain.addEventListener('touchstart', (e) => {
      // 입력 필드나 타이머 조작 중엔 무시
      if (e.target.tagName === 'INPUT' || e.target.tagName === 'BUTTON') return;
      execSwipeStartX = e.touches[0].clientX;
      execSwipeStartY = e.touches[0].clientY;
      execSwiping = true;
    }, { passive: true });

    execMain.addEventListener('touchmove', (e) => {
      if (!execSwiping) return;
      const dx = e.touches[0].clientX - execSwipeStartX;
      const dy = e.touches[0].clientY - execSwipeStartY;
      if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 10) {
        e.preventDefault();
      }
    }, { passive: false });

    execMain.addEventListener('touchend', (e) => {
      if (!execSwiping) return;
      execSwiping = false;
      const dx = e.changedTouches[0].clientX - execSwipeStartX;
      const dy = e.changedTouches[0].clientY - execSwipeStartY;
      if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 50) {
        if (dx < 0) {
          // 왼쪽 스와이프 → 다음 세트
          if (state.activeSetIndex < totalSets - 1) moveToSet(state.activeSetIndex + 1);
        } else {
          // 오른쪽 스와이프 → 이전 세트
          if (state.activeSetIndex > 0) moveToSet(state.activeSetIndex - 1);
        }
      }
    }, { passive: true });
  }
...
function renderReport() {
      ...
      record.sets.forEach((s, idx) => {
        ...
        setsStr += `
          <div style="display: flex; justify-content: space-between; padding: 4px 8px; border-bottom: 1px solid var(--border-color);">
            ...
            <div style="display: flex; align-items: center; gap: 8px;">
              <span style="font-weight: 500; font-size: 1.05rem;">${Number(s.weight).toLocaleString()}${state.weightUnit} x ${Number(s.reps).toLocaleString()}회</span>
        ...
        <div class="glass-panel" style="padding: 12px; border-radius: var(--radius-md);">
          <div style="font-weight: 700; color: var(--accent-primary); margin-bottom: 8px; font-size: 1.1rem; display: flex; justify-content: space-between; align-items: flex-end;">
            ...
            <span style="font-size: 0.85rem; color: var(--text-muted); font-weight: 500;">
              총 <span style="color:var(--text-primary); font-weight: bold; font-size: 1.05rem;">${totalVolume.toLocaleString()}${state.weightUnit}</span>
            </span>
          </div>
  ...
  document.getElementById('reportBackBtn').addEventListener('click', () => {
    ...
    } else if (state.reportSourceView === 'execution') {
      state.reportSourceView = null;
      navigateTo('execution');
    } else {
      navigateTo('workout');
...
// 부위별 볼륨 히트맵 헬퍼 — 종목 데이터(bodyPart) 기준 7개 부위로 매핑
function getZoneFromExerciseId(exerciseId) {
  const ex = state.exercises.find(e => e.id === exerciseId);
  const partName = (ex ? ex.bodyPart : '').toLowerCase();

  if (partName.includes('가슴') || partName.includes('chest')) return 'chest';
  if (partName.includes('등') || partName.includes('back')) return 'back';
  if (partName.includes('어깨') || partName.includes('shoulder')) return 'shoulder';
  if (partName.includes('이두') || partName.includes('biceps')) return 'biceps';
  if (partName.includes('삼두') || partName.includes('triceps')) return 'triceps';
  if (partName.includes('하체') || partName.includes('leg')) return 'leg';
  if (partName.includes('코어') || partName.includes('core') || partName.includes('복부')) return 'core';

  return 'other';
}

function buildBodyHeatmapSVG(bodyVol) {
  // 테마 accent 색상 읽기
  const hex = getComputedStyle(document.documentElement).getPropertyValue('--accent-primary').trim();
  let ar = 99, ag = 102, ab = 241;
  if (hex.startsWith('#') && hex.length >= 7) {
    ar = parseInt(hex.slice(1, 3), 16);
    ag = parseInt(hex.slice(3, 5), 16);
    ab = parseInt(hex.slice(5, 7), 16);
  }
  // 7개 부위로 합산
  const BODY_ZONES = ['chest', 'back', 'shoulder', 'biceps', 'triceps', 'leg', 'core'];
  const totalVol = BODY_ZONES.reduce((s, z) => s + (bodyVol[z] || 0), 0);
  function fc(zone) {
    const ratio = totalVol > 0 ? (bodyVol[zone] || 0) / totalVol : 0;
    const opacity = ratio <= 0 ? 0.09 : Math.min(0.95, 0.18 + ratio * 0.77);
    return `rgba(${ar},${ag},${ab},${opacity.toFixed(2)})`;
  }
  const base = `rgba(${ar},${ag},${ab},0.18)`;
  return `<svg viewBox="0 0 100 205" width="90" height="185" style="display:block;flex-shrink:0;">
    <ellipse cx="50" cy="14" rx="12" ry="13" fill="${base}"/>
    <rect x="45" y="25" width="10" height="9" rx="3" fill="${base}"/>
    <ellipse cx="50" cy="63" rx="28" ry="24" fill="${fc('back')}"/>
    <ellipse cx="28" cy="46" rx="13" ry="9" fill="${fc('shoulder')}"/>
    <ellipse cx="72" cy="46" rx="13" ry="9" fill="${fc('shoulder')}"/>
    <ellipse cx="39" cy="65" rx="12" ry="13" fill="${fc('chest')}"/>
    <ellipse cx="61" cy="65" rx="12" ry="13" fill="${fc('chest')}"/>
    <rect x="38" y="78" width="24" height="25" rx="5" fill="${fc('core')}"/>
    <!-- 왼쪽 상완: 외측(삼두), 내측(이두) -->
    <ellipse cx="16" cy="74" rx="4.5" ry="18" fill="${fc('triceps')}"/>
    <ellipse cx="24" cy="74" rx="4.5" ry="18" fill="${fc('biceps')}"/>
    <!-- 오른쪽 상완: 내측(이두), 외측(삼두) -->
    <ellipse cx="76" cy="74" rx="4.5" ry="18" fill="${fc('biceps')}"/>
    <ellipse cx="84" cy="74" rx="4.5" ry="18" fill="${fc('triceps')}"/>
    <!-- 전완 -->
    <ellipse cx="16" cy="109" rx="6" ry="13" fill="${base}"/>
    <ellipse cx="84" cy="109" rx="6" ry="13" fill="${base}"/>
    <ellipse cx="50" cy="106" rx="19" ry="8" fill="${fc('leg')}"/>
    <ellipse cx="37" cy="136" rx="13" ry="22" fill="${fc('leg')}"/>
    <ellipse cx="63" cy="136" rx="13" ry="22" fill="${fc('leg')}"/>
    <ellipse cx="36" cy="171" rx="9" ry="16" fill="${fc('leg')}"/>
    <ellipse cx="64" cy="171" rx="9" ry="16" fill="${fc('leg')}"/>
  </svg>`;
}
...
function renderWorkoutReport() {
  ...
  const prevExMax = {};
  const currExVol = {}; // { exId: totalVol } for pie chart
      ...
      ex.sets.forEach(s => {
        if (s.completed && s.weight > 0 && s.reps > 0) {
          ...
          if (isCurr) {
            ...
            currExVol[ex.exerciseId] = (currExVol[ex.exerciseId] || 0) + vol;
          }
          if (isPrev) {
            prevVol += vol;
            prevSets++;
  ...
  let volDiffStr = '';
  if (volDiff > 0) volDiffStr = `<span style="color:var(--success);">+${volDiff.toLocaleString()}${state.weightUnit}</span>`;
  else if (volDiff < 0) volDiffStr = `<span style="color:var(--danger);">${volDiff.toLocaleString()}${state.weightUnit}</span>`;
  else volDiffStr = `<span style="color:var(--text-muted);">-</span>`;

  const setsDiff = currSets - prevSets;
  let setsDiffStr = '';
  if (setsDiff > 0) setsDiffStr = `<span style="color:var(--success);">+${setsDiff}세트</span>`;
  else if (setsDiff < 0) setsDiffStr = `<span style="color:var(--danger);">${setsDiff}세트</span>`;
  else setsDiffStr = `<span style="color:var(--text-muted);">-</span>`;
  ...
  Object.keys(currExMax).forEach(exIdStr => {
    ...
    if (cMax > pMax) {
      ...
      const exName = dictEx ? dictEx.name : '알 수 없는 종목';
      let cmpStr = pMax === 0 ? `${cMax.toLocaleString()}${state.weightUnit}` : `${pMax.toLocaleString()}${state.weightUnit} → ${cMax.toLocaleString()}${state.weightUnit}`;
      increasedHtml += `
    ...
    <main>
      ...
      <div class="glass-panel" style="padding: 16px; margin-bottom: 20px;">
        <div style="display:flex; justify-content:center; align-items:center; gap: 12px; margin-bottom: 16px;">
          ...
          ${tab === 'day' ? `<button class="icon-btn" id="dayCalPickerBtn" style="padding:4px; background:var(--surface); border:1px solid var(--border-color);"><i class="ph ph-calendar-blank"></i></button><input type="date" id="dayCalInput" style="position:absolute;opacity:0;pointer-events:none;width:0;height:0;" value="${title}">` : ''}
        ...
        <div style="display:flex; justify-content:space-between; margin-bottom: 12px;">
          ...
          <span style="font-weight:bold; font-size:1.1rem;">${currVol.toLocaleString()} ${state.weightUnit}</span>
        </div>
        ${tab !== 'day' ? `
        <div style="display:flex; justify-content:space-between; margin-bottom: 12px;">
          <span style="color:var(--text-secondary);">전월 대비 볼륨</span>
          <span style="font-weight:bold; font-size:1.05rem;">${volDiffStr}</span>
        </div>` : ''}
        ${tab === 'day' ? `
        <div style="display:flex; justify-content:space-between; margin-bottom: 12px;">
          <span style="color:var(--text-secondary);">총 세트 수</span>
          <span style="font-weight:bold; font-size:1.1rem;">${currSets}세트</span>
        </div>` : ''}
        ${tab === 'month' ? `
        <div style="border-top:1px solid var(--border-color);margin-top:12px;padding-top:12px;">
          <div style="display:flex; justify-content:space-between; margin-bottom: 8px;">
            <span style="color:var(--text-secondary);">운동일</span>
        ...
        ` : ''}
        ${tab === 'day' ? `<div id="dayStatsSection"></div>` : ''}
        ${tab === 'week' ? `<div id="weekStatsSection"></div>` : ''}
        ${tab === 'month' ? `<div id="monthStatsSection"></div>` : ''}
      </div>
  ...
  document.getElementById('prevReportPeriodBtn').addEventListener('click', () => {
    ...
    if (tab === 'month') { d.setDate(1); d.setMonth(d.getMonth() - 1); }
  ...
  // 일별 탭 전용
  if (tab === 'day') {
    const calBtn = document.getElementById('dayCalPickerBtn');
    const calInput = document.getElementById('dayCalInput');
    if (calBtn && calInput) {
      calBtn.addEventListener('click', () => { calInput.showPicker ? calInput.showPicker() : calInput.click(); });
      calInput.addEventListener('change', (e) => {
        if (e.target.value) { state.reportDate = e.target.value; renderWorkoutReport(); }
      });
    }

    // 일별 통계 + 파이차트
    const daySection = document.getElementById('dayStatsSection');
    if (daySection) {
      const dayWorkout = state.workouts[sCurrStart];
      if (!dayWorkout || dayWorkout.exercises.length === 0) {
        daySection.innerHTML = `<div style="text-align:center; color:var(--text-muted); padding: 12px 0; font-size:0.9rem;">이 날 운동 기록이 없습니다.</div>`;
      } else {
        // 종목별 볼륨·세트 집계 + 그룹 정보
        const exStats = [];
        let totalSets = 0;
        const dayBodyVol = { chest: 0, back: 0, shoulder: 0, biceps: 0, triceps: 0, leg: 0, core: 0 };
        dayWorkout.exercises.forEach(ex => {
          const dictEx = state.exercises.find(e => e.id === ex.exerciseId);
          const name = dictEx ? dictEx.name : '알 수 없음';
          const grp = dictEx ? state.groups.find(g => g.id === dictEx.groupId) : null;
          let vol = 0, sets = 0;
          ex.sets.forEach(s => { if (s.completed) { sets++; vol += (s.weight || 0) * (s.reps || 0); } });
          totalSets += sets;
          if (sets > 0) {
            exStats.push({ name, vol, sets });
            const zone = getZoneFromExerciseId(ex.exerciseId);
            if (zone !== 'other') dayBodyVol[zone] = (dayBodyVol[zone] || 0) + vol;
          }
        });

        // 파이 차트 SVG
        const PIE_COLORS = ['#6366F1', '#10B981', '#F97316', '#EF4444', '#EAB308', '#8B5CF6', '#3B82F6', '#EC4899'];
        const totalVol = exStats.reduce((a, b) => a + b.vol, 0);
        let pieHtml = '';
        if (totalVol > 0) {
          const cx = 80, cy = 80, r = 70;
          let startAngle = -Math.PI / 2, slices = '';
          exStats.forEach((ex, i) => {
            const angle = (ex.vol / totalVol) * 2 * Math.PI;
            const x1 = cx + r * Math.cos(startAngle), y1 = cy + r * Math.sin(startAngle);
            const x2 = cx + r * Math.cos(startAngle + angle), y2 = cy + r * Math.sin(startAngle + angle);
            const large = angle > Math.PI ? 1 : 0;
            slices += `<path d="M${cx},${cy} L${x1.toFixed(2)},${y1.toFixed(2)} A${r},${r} 0 ${large},1 ${x2.toFixed(2)},${y2.toFixed(2)} Z" fill="${PIE_COLORS[i % PIE_COLORS.length]}" opacity="0.85"/>`;
            startAngle += angle;
          });
          pieHtml = `<svg viewBox="0 0 160 160" width="100%" style="display:block;">${slices}</svg>`;
        }

        // 범례 2열 그리드
        let legendHtml = exStats.map((ex, i) => {
          const pct = totalVol > 0 ? ((ex.vol / totalVol) * 100).toFixed(1) : 0;
          const col = PIE_COLORS[i % PIE_COLORS.length];
          return `<div style="display:flex;align-items:center;gap:6px;padding:4px 0;min-width:0;overflow:hidden;">
            <span style="width:10px;height:10px;border-radius:50%;background:${col};flex-shrink:0;"></span>
            <span style="font-size:0.82rem;font-weight:600;flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${ex.name}</span>
            <span style="font-size:0.78rem;color:var(--accent-primary);font-weight:700;white-space:nowrap;flex-shrink:0;">${pct}%</span>
          </div>`;
        }).join('');

        const bodyMapHtml = buildBodyHeatmapSVG(dayBodyVol);

        daySection.innerHTML = `
          <div style="border-top:1px solid var(--border-color);margin-top:12px;padding-top:12px;">
            <div style="display:flex;gap:10px;align-items:flex-start;margin-bottom:10px;">
              <div style="flex:1;min-width:0;">${pieHtml}</div>
              ${bodyMapHtml}
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px 8px;">${legendHtml}</div>
          </div>`;
      }
    }
  }
  // 월별 탭 전용
  if (tab === 'month') {
    const monthSection = document.getElementById('monthStatsSection');
    if (monthSection) {
      // 세트 수 영역
      let html = `
        <div style="border-top:1px solid var(--border-color);margin-top:12px;padding-top:12px;">
          <div style="display:flex;justify-content:space-between;margin-bottom:8px;">
            <span style="color:var(--text-secondary);">총 세트 수</span><span style="font-weight:bold;">${currSets}세트</span>
          </div>
          <div style="display:flex;justify-content:space-between;">
            <span style="color:var(--text-secondary);">전월 대비 세트</span><span style="font-weight:bold;font-size:1.05rem;">${setsDiffStr}</span>
          </div>
        </div>`;

      // 최고 중량 갱신 (전월 대비)
      let mIncHtml = '', mIncCnt = 0;
      Object.keys(currExMax).forEach(exIdStr => {
        const exId = parseInt(exIdStr);
        const cMax = currExMax[exId];
        const pMax = prevExMax[exId] || 0;
        if (cMax > pMax) {
          ...
          const dictEx = state.exercises.find(e => e.id === exId);
          const exName = dictEx ? dictEx.name : '알 수 없는 종목';
          const newTag = pMax === 0 ? ` <span style="color:var(--accent-primary);font-size:0.8rem;">(신규!!)</span>` : '';
          const cmpStr = pMax === 0 ? `${cMax}${state.weightUnit}${newTag}` : `${pMax}${state.weightUnit} → ${cMax}${state.weightUnit}`;
          mIncHtml += `<div style="display:flex;justify-content:space-between;align-items:center;padding:7px 0;border-bottom:1px solid var(--border-color);">
            <span style="font-weight:600;">${exName}</span>
            <span style="color:var(--success);font-weight:600;font-size:0.92rem;">${cmpStr}</span>
          </div>`;
        }
      });
      if (mIncCnt === 0) mIncHtml = `<div class="info-text" style="font-size:0.85rem;padding:6px 0;">이번 달 증량한 종목이 없습니다.</div>`;
      html += `
        <div style="border-top:1px solid var(--border-color);margin-top:14px;padding-top:12px;">
          <div style="font-size:0.9rem;font-weight:700;color:var(--accent-primary);margin-bottom:8px;">증량 종목(${mIncCnt}개)</div>
          ${mIncHtml}
        </div>`;

      // 파이 차트 + 부위별 히트맵
      const MPC = ['#6366F1', '#10B981', '#F97316', '#EF4444', '#EAB308', '#8B5CF6', '#3B82F6', '#EC4899'];
      const mEvl = Object.keys(currExVol).map(id => { const e = state.exercises.find(e => e.id === parseInt(id)); return { name: e ? e.name : '알 수 없음', vol: currExVol[id], exId: parseInt(id) }; }).filter(e => e.vol > 0).sort((a, b) => b.vol - a.vol);
      const mTpv = mEvl.reduce((a, b) => a + b.vol, 0);
      if (mTpv > 0) {
        const mBodyVol = { chest: 0, back: 0, shoulder: 0, biceps: 0, triceps: 0, leg: 0, core: 0 };
        mEvl.forEach(e => { const z = getZoneFromExerciseId(e.exId); if (z !== 'other') mBodyVol[z] = (mBodyVol[z] || 0) + e.vol; });
        const cx = 80, cy = 80, r = 70; let sa = -Math.PI / 2, sl = '';
        mEvl.forEach((ex, i) => {
          const ang = (ex.vol / mTpv) * 2 * Math.PI;
          const x1 = cx + r * Math.cos(sa), y1 = cy + r * Math.sin(sa), x2 = cx + r * Math.cos(sa + ang), y2 = cy + r * Math.sin(sa + ang);
          sl += `<path d="M${cx},${cy} L${x1.toFixed(2)},${y1.toFixed(2)} A${r},${r} 0 ${ang > Math.PI ? 1 : 0},1 ${x2.toFixed(2)},${y2.toFixed(2)} Z" fill="${MPC[i % MPC.length]}" opacity="0.85"/>`;
          sa += ang;
        });
        const lg = mEvl.map((ex, i) => `<div style="display:flex;align-items:center;gap:6px;padding:4px 0;min-width:0;overflow:hidden;"><span style="width:10px;height:10px;border-radius:50%;background:${MPC[i % MPC.length]};flex-shrink:0;"></span><span style="font-size:0.82rem;font-weight:600;flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${ex.name}</span><span style="font-size:0.78rem;color:var(--accent-primary);font-weight:700;white-space:nowrap;flex-shrink:0;">${((ex.vol / mTpv) * 100).toFixed(1)}%</span></div>`).join('');
        html += `
          <div style="border-top:1px solid var(--border-color);margin-top:14px;padding-top:12px;">
            <div style="font-size:0.9rem;font-weight:700;color:var(--accent-primary);margin-bottom:8px;">월간 볼륨 분포</div>
            <div style="display:flex;gap:10px;align-items:flex-start;margin-bottom:10px;">
              <div style="flex:1;min-width:0;"><svg viewBox="0 0 160 160" width="100%" style="display:block;">${sl}</svg></div>
              ${buildBodyHeatmapSVG(mBodyVol)}
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px 8px;">${lg}</div>
          </div>`;
      }
  ...
  // 주별 탭 전용
  if (tab === 'week') {
    const weekSection = document.getElementById('weekStatsSection');
    if (weekSection) {
      // 세트 수 영역
      let html = `
        <div style="border-top:1px solid var(--border-color);margin-top:12px;padding-top:12px;">
          ...
          <div style="display:flex;justify-content:space-between;">
            <span style="color:var(--text-secondary);">전주 대비 세트</span><span style="font-weight:bold;font-size:1.05rem;">${setsDiffStr}</span>
          </div>
        </div>`;

      // 최고 중량 갱신 영역
      const allDates = Object.keys(state.workouts).sort();
      let wIncHtml = '', wIncCnt = 0;
      Object.keys(currExMax).forEach(exIdStr => {
        const exId = parseInt(exIdStr);
        const cMax = currExMax[exId];
        let prevMax = 0;
        for (let i = allDates.length - 1; i >= 0; i--) {
          const d = allDates[i]; if (d >= sCurrStart) continue;
          const w = state.workouts[d]; let m = 0;
          w.exercises.forEach(ex => { if (ex.exerciseId === exId) ex.sets.forEach(s => { if (s.completed && s.weight > m) m = s.weight; }); });
          if (m > 0) { prevMax = m; break; }
        }
        if (cMax > prevMax) {
          wIncCnt++;
          const dictEx = state.exercises.find(e => e.id === exId);
          const exName = dictEx ? dictEx.name : '알 수 없는 종목';
          const newTag = prevMax === 0 ? ` <span style="color:var(--accent-primary);font-size:0.8rem;">(신규!!)</span>` : '';
          const cmpStr = prevMax === 0 ? `${cMax}${state.weightUnit}${newTag}` : `${prevMax}${state.weightUnit} → ${cMax}${state.weightUnit}`;
          wIncHtml += `<div style="display:flex;justify-content:space-between;align-items:center;padding:7px 0;border-bottom:1px solid var(--border-color);">
            <span style="font-weight:600;">${exName}</span>
            <span style="color:var(--success);font-weight:600;font-size:0.92rem;">${cmpStr}</span>
          </div>`;
        }
      });
      if (wIncCnt === 0) wIncHtml = `<div class="info-text" style="font-size:0.85rem;padding:6px 0;">이번 주 최고 중량이 갱신된 종목이 없습니다.</div>`;
      html += `
        <div style="border-top:1px solid var(--border-color);margin-top:14px;padding-top:12px;">
          <div style="font-size:0.9rem;font-weight:700;color:var(--accent-primary);margin-bottom:8px;">최고 중량 갱신 종목 (${wIncCnt}개)</div>
          ${wIncHtml}
        </div>`;

      // 파이 차트 + 부위별 히트맵
      const PC = ['#6366F1', '#10B981', '#F97316', '#EF4444', '#EAB308', '#8B5CF6', '#3B82F6', '#EC4899'];
      const evl = Object.keys(currExVol).map(id => { const e = state.exercises.find(e => e.id === parseInt(id)); return { name: e ? e.name : '알 수 없음', vol: currExVol[id], exId: parseInt(id) }; }).filter(e => e.vol > 0).sort((a, b) => b.vol - a.vol);
      const tpv = evl.reduce((a, b) => a + b.vol, 0);
      if (tpv > 0) {
        const wBodyVol = { chest: 0, back: 0, shoulder: 0, biceps: 0, triceps: 0, leg: 0, core: 0 };
        evl.forEach(e => { const z = getZoneFromExerciseId(e.exId); if (z !== 'other') wBodyVol[z] = (wBodyVol[z] || 0) + e.vol; });
        const cx = 80, cy = 80, r = 70; let sa = -Math.PI / 2, sl = '';
        evl.forEach((ex, i) => {
          const ang = (ex.vol / tpv) * 2 * Math.PI;
          const x1 = cx + r * Math.cos(sa), y1 = cy + r * Math.sin(sa), x2 = cx + r * Math.cos(sa + ang), y2 = cy + r * Math.sin(sa + ang);
          sl += `<path d="M${cx},${cy} L${x1.toFixed(2)},${y1.toFixed(2)} A${r},${r} 0 ${ang > Math.PI ? 1 : 0},1 ${x2.toFixed(2)},${y2.toFixed(2)} Z" fill="${PC[i % PC.length]}" opacity="0.85"/>`;
          sa += ang;
        });
        const lg = evl.map((ex, i) => `<div style="display:flex;align-items:center;gap:6px;padding:4px 0;min-width:0;overflow:hidden;"><span style="width:10px;height:10px;border-radius:50%;background:${PC[i % PC.length]};flex-shrink:0;"></span><span style="font-size:0.82rem;font-weight:600;flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${ex.name}</span><span style="font-size:0.78rem;color:var(--accent-primary);font-weight:700;white-space:nowrap;flex-shrink:0;">${((ex.vol / tpv) * 100).toFixed(1)}%</span></div>`).join('');
        html += `
          <div style="border-top:1px solid var(--border-color);margin-top:14px;padding-top:12px;">
            <div style="font-size:0.9rem;font-weight:700;color:var(--accent-primary);margin-bottom:8px;">주간 볼륨 분포</div>
            <div style="display:flex;gap:10px;align-items:flex-start;margin-bottom:10px;">
              <div style="flex:1;min-width:0;"><svg viewBox="0 0 160 160" width="100%" style="display:block;">${sl}</svg></div>
              ${buildBodyHeatmapSVG(wBodyVol)}
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px 8px;">${lg}</div>
          </div>`;
      }
  ...
  appEl.innerHTML = `
    ...
    <main style="padding: 20px; font-size: 0.95rem; line-height: 1.6; overflow-y: auto;">
      ...
      <div style="margin-bottom: 25px; padding: 0 5px;">
        <h2 style="font-size: 1.3rem; font-weight: 800; color: var(--text-primary); margin-bottom: 8px;">GUMBA's WorkOut</h2>
        <p style="color: var(--text-secondary); font-size: 0.9rem;">
          이 앱은 과학적인 프로그래밍과 데이터 기반의 성장을 위해 설계된 당신의 디지털 코치입니다. 
          최고의 퍼포먼스를 끌어내기 위한 핵심 가이드를 숙지하세요.
        </p>
      </div>

      <div class="glass-panel" style="padding: 18px; margin-bottom: 20px; border-left: 4px solid var(--accent-primary);">
        <h3 style="color: var(--accent-primary); margin-bottom: 10px; font-size: 1.05rem; display: flex; align-items: center; gap: 8px;">
          <i class="ph-fill ph-calendar-check" style="flex-shrink:0;"></i> 1. 성장의 시작: 루틴 설계
        </h3>
        <p style="color: var(--text-secondary); margin: 0; font-size: 0.9rem;">
          모든 변화는 계획에서 시작됩니다. 달력에서 날짜를 선택하고 <b>[새로운 루틴 시작]</b>을 누르세요. 
          엄선된 32개의 필수 종목과 부위별 프리셋 그룹이 당신의 기초를 단단하게 다져줄 것입니다.
        </p>
      </div>

      <div class="glass-panel" style="padding: 18px; margin-bottom: 20px; border-left: 4px solid var(--accent-primary);">
        <h3 style="color: var(--accent-primary); margin-bottom: 10px; font-size: 1.05rem; display: flex; align-items: center; gap: 8px;">
          <i class="ph-fill ph-barbell" style="flex-shrink:0;"></i> 2. 퍼포먼스 데이터의 정밀 기록
        </h3>
        <p style="color: var(--text-secondary); margin: 0; font-size: 0.9rem;">
          훈련은 정교해야 합니다. 종목 카드 우측의 <b>[진행 아이콘(>)]</b>을 눌러 실행 화면으로 진입하세요. 
          세트 완료 시 데이터는 즉시 보존되며, 당신의 모든 땀방울은 객관적인 숫자로 기록됩니다.
        </p>
      </div>

      <div class="glass-panel" style="padding: 18px; margin-bottom: 20px; border-left: 4px solid var(--success);">
        <h3 style="color: var(--success); margin-bottom: 10px; font-size: 1.05rem; display: flex; align-items: center; gap: 8px;">
          <i class="ph-fill ph-timer" style="flex-shrink:0;"></i> 3. 휴식: 성장의 골든타임
        </h3>
        <p style="color: var(--text-secondary); margin: 0; font-size: 0.9rem;">
          근육은 훈련 중이 아니라 휴식 중에 성장합니다. <b>[휴식]</b> 버튼을 통해 회복 시간을 과학적으로 통제하세요. 
          타이머 종료 알림과 진동은 다음 전장으로 복귀하라는 코치의 신호입니다.
        </p>
      </div>

      <div class="glass-panel" style="padding: 18px; margin-bottom: 20px; border-left: 4px solid var(--accent-primary);">
        <h3 style="color: var(--accent-primary); margin-bottom: 10px; font-size: 1.05rem; display: flex; align-items: center; gap: 8px;">
          <i class="ph-fill ph-chart-line-up" style="flex-shrink:0;"></i> 4. 데이터로 증명하는 과부하
        </h3>
        <p style="color: var(--text-secondary); margin: 0; font-size: 0.9rem;">
          느낌에 의존하지 마세요. <b>[통계 리포트]</b>는 당신의 소화 볼륨(Weight × Reps)과 최고 중량 갱신 여부를 추적합니다. 
          숫자는 거짓말을 하지 않으며, 성장의 지도를 그려줄 것입니다.
        </p>
      </div>
      
      <div class="glass-panel" style="padding: 18px; margin-bottom: 20px; border-left: 4px solid var(--warning);">
        <h3 style="color: var(--warning); margin-bottom: 10px; font-size: 1.05rem; display: flex; align-items: center; gap: 8px;">
          <i class="ph-fill ph-calculator" style="flex-shrink:0;"></i> 5. 5/3/1: 엘리트의 시스템
        </h3>
        <p style="color: var(--text-secondary); margin: 0; font-size: 0.9rem;">
          짐 웬들러의 5/3/1은 정점에 도달하기 위한 시스템입니다. 1RM을 입력하면 당신의 <b>Training Max(TM)</b>에 맞춘 주차별 계획이 자동 산출됩니다. 
          사이클 조절 버튼으로 한계를 돌파하세요.
        </p>
      </div>

      <div class="glass-panel" style="padding: 18px; margin-bottom: 20px; border-left: 4px solid var(--accent-primary);">
        <h3 style="color: var(--accent-primary); margin-bottom: 10px; font-size: 1.05rem; display: flex; align-items: center; gap: 8px;">
          <i class="ph-fill ph-clock-counter-clockwise" style="flex-shrink:0;"></i> 6. 커스텀 템포와 디테일
        </h3>
        <p style="color: var(--text-secondary); margin: 0; font-size: 0.9rem;">
          고급 트레이닝일수록 세밀한 조정이 필요합니다. 실행 화면에서 휴식 시간을 수정하면 해당 종목의 <b>고유 설정</b>으로 즉시 반영됩니다. 
          당신의 몸에 맞는 최적의 훈련 템포를 구축하세요.
        </p>
      </div>

      <div class="glass-panel" style="padding: 18px; margin-bottom: 20px; border-left: 4px solid var(--danger);">
        <h3 style="color: var(--danger); margin-bottom: 10px; font-size: 1.05rem; display: flex; align-items: center; gap: 8px;">
          <i class="ph-fill ph-floppy-disk" style="flex-shrink:0;"></i> 7. 훈련 자산의 영속성
        </h3>
        <p style="color: var(--text-secondary); margin: 0; font-size: 0.9rem;">
          당신의 훈련 데이터는 무엇과도 바꿀 수 없는 자산입니다. 기기 변경 시 <b>[데이터 백업]</b>을 통해 당신의 역사를 보존하세요. 
          클라우드보다 안전한 당신만의 로컬 데이터베이스입니다.
        </p>
      </div>

      <div class="glass-panel" style="padding: 18px; margin-bottom: 30px; border-left: 4px solid var(--success);">
        <h3 style="color: var(--success); margin-bottom: 10px; font-size: 1.05rem; display: flex; align-items: center; gap: 8px;">
          <i class="ph-fill ph-heartbeat" style="flex-shrink:0;"></i> 8. 체성분: 숫자가 아닌 경향성
        </h3>
        <p style="color: var(--text-secondary); margin: 0; font-size: 0.9rem;">
          단순한 체중이 아닌 '기울기'를 보십시오. 인바디 기록은 BMI와 각 지표의 변화 추이를 시각화합니다. 
          <b>듀얼 축 그래프</b>의 선들이 우상향할 때, 당신의 노력은 비로소 증명됩니다.
        </p>
      </div>

      <div style="text-align: center; padding: 20px 0; opacity: 0.6; font-size: 0.8rem;">
        &copy; Developed by Master Coach & Senior Engineer
      </div>
...
function renderFiveThreeOne() {
  ...
  const calculateSets = (tm, week) => {
    ...
    return percs.map((p, i) => ({ weight: roundToStep(tm * p, state.fiveThreeOneRoundStep), reps: reps[i] }));
  };
          ...
          <div style="display: flex; flex-direction: column; gap: 6px;">
            ${sets.map(s => `
              <div style="font-size: ${setsFs}; display: flex; justify-content: space-between; align-items: center;">
                <span style="font-weight: 500;">${s.weight}${state.weightUnit}</span>
  ...
  const updateOnlyValues = () => {
    ...
    document.querySelectorAll('.round-step-btn').forEach(btn => {
      btn.classList.toggle('active', parseFloat(btn.getAttribute('data-step')) === state.fiveThreeOneRoundStep);
    });
  ...
  if (anyRmEntered) {
    ...
    weeksHtml = `
      <div class="glass-panel" style="padding: 40px 20px; text-align: center; opacity: 0.8; margin-bottom: 20px;">
        <i class="ph ph-info" style="font-size: 2.5rem; margin-bottom: 15px; display: block; color: var(--accent-primary);"></i>
        <div style="font-size: 1rem; line-height: 1.5; color: var(--text-primary);">
          S, B, D, O 중 <span style="color: var(--accent-primary); font-weight: 700;">하나 이상의 1RM</span>을<br>
          입력하면 일주일 단위 계산이 시작됩니다.
        </div>
      </div>
    `;
      ...
      <div class="glass-panel" style="padding: 20px; margin-bottom: 25px;">
        <div style="margin-bottom: 25px;">
          ...
          <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; margin-bottom: 15px;">
            <div class="form-group" style="display: flex; flex-direction: column; align-items: center; margin-bottom: 12px;">
              <label style="font-size: 0.75rem; margin-bottom: 4px; text-align: center; width: 100%;">S</label>
              <input type="number" id="rmSquat" class="form-control" value="${rms.squat || ''}" placeholder="0" step="2.5" style="padding: 8px 4px; text-align: center; margin-bottom: 4px; width: 100%; max-width: 80px;">
              <div style="font-size: 0.7rem; color: var(--accent-primary); font-weight: 700; text-align: center; width: 100%;">TM: ${rms.squat ? (rms.squat * 0.9).toFixed(1) : '-'}</div>
            </div>
            <div class="form-group" style="display: flex; flex-direction: column; align-items: center; margin-bottom: 12px;">
              <label style="font-size: 0.75rem; margin-bottom: 4px; text-align: center; width: 100%;">B</label>
              <input type="number" id="rmBench" class="form-control" value="${rms.bench || ''}" placeholder="0" step="2.5" style="padding: 8px 4px; text-align: center; margin-bottom: 4px; width: 100%; max-width: 80px;">
              <div style="font-size: 0.7rem; color: var(--accent-primary); font-weight: 700; text-align: center; width: 100%;">TM: ${rms.bench ? (rms.bench * 0.9).toFixed(1) : '-'}</div>
            </div>
            <div class="form-group" style="display: flex; flex-direction: column; align-items: center; margin-bottom: 12px;">
              <label style="font-size: 0.75rem; margin-bottom: 4px; text-align: center; width: 100%;">D</label>
              <input type="number" id="rmDead" class="form-control" value="${rms.deadlift || ''}" placeholder="0" step="2.5" style="padding: 8px 4px; text-align: center; margin-bottom: 4px; width: 100%; max-width: 80px;">
              <div style="font-size: 0.7rem; color: var(--accent-primary); font-weight: 700; text-align: center; width: 100%;">TM: ${rms.deadlift ? (rms.deadlift * 0.9).toFixed(1) : '-'}</div>
            </div>
            <div class="form-group" style="display: flex; flex-direction: column; align-items: center; margin-bottom: 12px;">
              <label style="font-size: 0.75rem; margin-bottom: 4px; text-align: center; width: 100%;">O</label>
              <input type="number" id="rmOhp" class="form-control" value="${rms.ohp || ''}" placeholder="0" step="2.5" style="padding: 8px 4px; text-align: center; margin-bottom: 4px; width: 100%; max-width: 80px;">
              <div style="font-size: 0.7rem; color: var(--accent-primary); font-weight: 700; text-align: center; width: 100%;">TM: ${rms.ohp ? (rms.ohp * 0.9).toFixed(1) : '-'}</div>
            </div>
          </div>

          <div style="display: flex; gap: 8px; margin-bottom: 15px;">
            <button class="btn" id="decCycleBtn" style="flex: 1; padding: 10px; font-size: 0.85rem; border-radius: 8px; background: rgba(255,255,255,0.05); color: var(--text-secondary); font-weight: 600; justify-content: center;">
              <i class="ph ph-minus-circle"></i> 이전 사이클 (-2.5)
            </button>
            <button class="btn btn-primary" id="incCycleBtn" style="flex: 1.5; padding: 10px; font-size: 0.85rem; border-radius: 8px; font-weight: 700; justify-content: center; box-shadow: var(--shadow-glow);">
              <i class="ph ph-plus-circle"></i> 다음 사이클 (+2.5)
            </button>
          </div>
        </div>
        
        <div style="margin-bottom: 25px;">
          <h3 style="font-size: 1rem; margin-bottom: 15px;">무게 반올림 단위</h3>
          <div class="mode-toggle">
            <div class="mode-btn round-step-btn ${state.fiveThreeOneRoundStep === 2.5 ? 'active' : ''}" data-step="2.5">2.5 단위</div>
            <div class="mode-btn round-step-btn ${state.fiveThreeOneRoundStep === 5 ? 'active' : ''}" data-step="5">5 단위</div>
          </div>
        </div>
        ...
        <div>
          ...
          <div class="mode-toggle">
            <div class="mode-btn option-btn ${option === 'A' ? 'active' : ''}" data-opt="A">4주</div>
            <div class="mode-btn option-btn ${option === 'B' ? 'active' : ''}" data-opt="B">7주</div>
          </div>
  ...
  document.getElementById('incCycleBtn').addEventListener('click', () => {
    if (state.fiveThreeOneRms.squat > 0) state.fiveThreeOneRms.squat += 2.5;
    if (state.fiveThreeOneRms.deadlift > 0) state.fiveThreeOneRms.deadlift += 2.5;
    if (state.fiveThreeOneRms.bench > 0) state.fiveThreeOneRms.bench += 2.5;
    if (state.fiveThreeOneRms.ohp > 0) state.fiveThreeOneRms.ohp += 2.5;
    localStorage.setItem('fiveThreeOneRms', JSON.stringify(state.fiveThreeOneRms));
    renderFiveThreeOne();
  });

  document.getElementById('decCycleBtn').addEventListener('click', () => {
    if (state.fiveThreeOneRms.squat > 0) state.fiveThreeOneRms.squat = Math.max(0, state.fiveThreeOneRms.squat - 2.5);
    if (state.fiveThreeOneRms.deadlift > 0) state.fiveThreeOneRms.deadlift = Math.max(0, state.fiveThreeOneRms.deadlift - 2.5);
    if (state.fiveThreeOneRms.bench > 0) state.fiveThreeOneRms.bench = Math.max(0, state.fiveThreeOneRms.bench - 2.5);
    if (state.fiveThreeOneRms.ohp > 0) state.fiveThreeOneRms.ohp = Math.max(0, state.fiveThreeOneRms.ohp - 2.5);
    localStorage.setItem('fiveThreeOneRms', JSON.stringify(state.fiveThreeOneRms));
    renderFiveThreeOne();
  });
  ...
  document.querySelectorAll('.round-step-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      state.fiveThreeOneRoundStep = parseFloat(e.target.getAttribute('data-step'));
      localStorage.setItem('fiveThreeOneRoundStep', state.fiveThreeOneRoundStep);
      renderFiveThreeOne();
    });
  });
  ...
  if (w > 0 && r > 0) {
    ...
    resultsHtml = `
      <div class="glass-panel" style="padding: 20px; margin-bottom: 20px;">
        <h3 style="font-size: 1.1rem; color: var(--accent-primary); margin: 0 0 20px 0; text-align: center;">추정 결과 (단위: ${state.weightUnit})</h3>
    ...
    <main>
      <div class="glass-panel" style="padding: 20px; margin-bottom: 25px;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
          <div class="form-group">
            <label>수행 중량 (${state.weightUnit})</label>
      ...
      listHtml += `
        <div class="glass-panel" style="padding: 16px; margin-bottom: 12px; position: relative;">
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; font-size: 0.95rem;">
            <div style="color: var(--text-secondary);">체중: <span style="color: var(--text-primary); font-weight: 600;">${rec.weight}${state.weightUnit}</span>${getDeltaHtml(rec.weight, prev ? prev.weight : null, 'weight')}</div>
            ...
            <div style="color: var(--text-secondary);">골격근: <span style="color: var(--text-primary); font-weight: 600;">${rec.skeletalMuscle || 0}${state.weightUnit}</span>${getDeltaHtml(rec.skeletalMuscle, prev ? prev.skeletalMuscle : null, 'skeletalMuscle')}</div>
            <div style="color: var(--text-secondary);">체지방: <span style="color: var(--text-primary); font-weight: 600;">${rec.bodyFatMass || 0}${state.weightUnit}</span>${getDeltaHtml(rec.bodyFatMass, prev ? prev.bodyFatMass : null, 'bodyFatMass')}</div>
...
function renderKgLbConverter() {
  let tableHtml = `
    <table style="width: 100%; border-collapse: collapse; margin-top: 20px; text-align: center; color: var(--text-primary); font-size: 0.85rem;">
      <thead>
        <tr style="background: rgba(255, 255, 255, 0.05);">
          <th style="padding: 8px; border: 1px solid var(--border-color);">LBS</th>
          <th style="padding: 8px; border: 1px solid var(--border-color);">KG</th>
          <th style="padding: 8px; border: 1px solid var(--border-color);">LBS</th>
          <th style="padding: 8px; border: 1px solid var(--border-color);">KG</th>
        </tr>
      </thead>
  ...
  for (let lbs = 10; lbs <= 100; lbs += 10) {
    const kg1 = (lbs * 0.453592).toFixed(1);
    const lbs2 = lbs + 100;
    const kg2 = (lbs2 * 0.453592).toFixed(1);
    tableHtml += `
        <tr>
          <td style="padding: 8px; border: 1px solid var(--border-color); font-weight: 600; color: var(--accent-primary);">${lbs}</td>
          <td style="padding: 8px; border: 1px solid var(--border-color); color: var(--text-secondary);">${kg1}</td>
          <td style="padding: 8px; border: 1px solid var(--border-color); font-weight: 600; color: var(--accent-primary);">${lbs2}</td>
          <td style="padding: 8px; border: 1px solid var(--border-color); color: var(--text-secondary);">${kg2}</td>
        </tr>
    `;
  }
  tableHtml += `
  ...
  appEl.innerHTML = `
    <header>
      <button class="icon-btn" id="closeKgLbBtn"><i class="ph ph-caret-left"></i></button>
      <h1>kg-lbs 변환기</h1>
      <button class="icon-btn" style="visibility: hidden;"><i class="ph ph-caret-left"></i></button>
    </header>
    <main>
      <div class="glass-panel" style="padding: 20px;">
        <h2 style="font-size: 1.1rem; margin-bottom: 16px; color: var(--text-primary); text-align: center;"></h2>
        <div style="display: flex; gap: 10px; margin-bottom: 20px;">
          <div style="flex: 1;">
            <label style="display: block; font-size: 0.85rem; color: var(--text-muted); margin-bottom: 4px; text-align: center;">KG 입력</label>
            <input type="number" id="kgInput" class="input-field" placeholder="입력 (kg)" style="width: 100%; box-sizing: border-box; height: 56px; font-size: 1.2rem; text-align: center; font-weight: 600; padding: 12px; border: 2px solid var(--border-color);">
          </div>
          <div style="flex: 1;">
            <label style="display: block; font-size: 0.85rem; color: var(--accent-primary); margin-bottom: 4px; font-weight: bold; text-align: center;">LBS 결과</label>
            <input type="text" id="lbsOutput" class="input-field" placeholder="결과 (lbs)" readonly style="width: 100%; box-sizing: border-box; height: 56px; background: var(--surface-hover); color: var(--accent-primary); font-weight: 800; font-size: 1.2rem; border: 2px solid var(--accent-primary); padding: 12px; text-align: center; opacity: 1; -webkit-text-fill-color: var(--accent-primary);">
          </div>
        </div>
        <div style="display: flex; gap: 10px; margin-bottom: 20px;">
          <div style="flex: 1;">
            <label style="display: block; font-size: 0.85rem; color: var(--text-muted); margin-bottom: 4px; text-align: center;">LBS 입력</label>
            <input type="number" id="lbsInput" class="input-field" placeholder="입력 (lbs)" style="width: 100%; box-sizing: border-box; height: 56px; font-size: 1.2rem; text-align: center; font-weight: 600; padding: 12px; border: 2px solid var(--border-color);">
          </div>
          <div style="flex: 1;">
            <label style="display: block; font-size: 0.85rem; color: var(--accent-primary); margin-bottom: 4px; font-weight: bold; text-align: center;">KG 결과</label>
            <input type="text" id="kgOutput" class="input-field" placeholder="결과 (kg)" readonly style="width: 100%; box-sizing: border-box; height: 56px; background: var(--surface-hover); color: var(--accent-primary); font-weight: 800; font-size: 1.2rem; border: 2px solid var(--accent-primary); padding: 12px; text-align: center; opacity: 1; -webkit-text-fill-color: var(--accent-primary);">
          </div>
        </div>

        <h2 style="font-size: 1.1rem; margin-top: 30px; margin-bottom: 10px; color: var(--text-primary); text-align: center;">변환표(10~200lbs)</h2>
        ${tableHtml}
      </div>
    </main>
  `;

  document.getElementById('closeKgLbBtn').addEventListener('click', () => {
    navigateTo('settings');
  });
  ...
  kgInput.addEventListener('input', (e) => {
    ...
    if (!isNaN(val)) {
      lbsOutput.value = (val * 2.20462262).toFixed(1);
    } else {
      lbsOutput.value = '';
    }
  });
  ...
  lbsInput.addEventListener('input', (e) => {
    const val = parseFloat(e.target.value);
    if (!isNaN(val)) {
      kgOutput.value = (val * 0.45359237).toFixed(1);
    } else {
      kgOutput.value = '';
    }
  });
